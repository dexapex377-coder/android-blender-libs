...
/// returns "".
 std::string parent_path (string_view filepath) noexcept;
...
/// Input and output are both UTF-8 encoded.
 std::string generic_filepath (string_view filepath) noexcept;
...
/// to be UTF-8 encoded.
 std::string searchpath_find (const std::string &filename,
...
/// If the program is not found, return an empty string.
 std::string find_program(string_view program);
...
/// and directory names are presumed to be UTF-8 encoded.
 bool get_directory_entries (const std::string &dirname,
...
/// success, false for failure and place an error message in err.
 bool create_directory (string_view path, std::string &err);
...
/// failure and place an error message in err.
 bool copy (string_view from, string_view to, std::string &err);
...
/// message in err.
 bool rename (string_view from, string_view to, std::string &err);
...
/// true for success, false for failure and place an error message in err.
 bool remove (string_view path, std::string &err);
...
/// Place an error message (if applicable in err.
 unsigned long long remove_all (string_view path, std::string &err);
...
/// not an emergency. We'll replace this with something else eventually.
 std::string unique_path (string_view model="%%%%-%%%%-%%%%-%%%%");

/// Version of fopen that can handle UTF-8 paths even on Windows.
 FILE *fopen (string_view path, string_view mode);
...
///
 void open (OIIO::ifstream &stream, string_view path,
                    std::ios_base::openmode mode = std::ios_base::in);

/// Version of std::ofstream.open that can handle UTF-8 paths
///
 void open (OIIO::ofstream &stream, string_view path,
...
/// `#include <fcntl.h>` in order to get the definitions of the flags.)
 int open (string_view path, int flags);
...
/// failure.
     bool read_text_file(string_view filename, std::string &str,
                                      size_t size = (1UL << 24));
     bool read_text_from_command(string_view command,
                                              std::string &str,
                                              size_t size = (1UL << 24));
...
/// returning true on success, false on failure.
 bool write_text_file (string_view filename, string_view str);
...
/// to generate a sequence.
 bool enumerate_sequence (string_view desc,
...
/// Return true upon success, false if the description was too malformed
/// to generate a sequence.
 bool parse_pattern (const char *pattern,
...
/// Return true upon success, false if the description was too malformed
/// to generate a sequence.
 bool enumerate_file_sequence (const std::string &pattern,
...
/// pattern can't be parsed.
 bool scan_for_matching_filenames (const std::string &pattern,
                                           const std::vector<string_view> &views,
...
/// more characters.
 std::string filename_to_regex(string_view pattern,
...
/// grow as we write.
class  IOVecOutput : public IOProxy {
public:
...
class  IOMemReader : public IOProxy {
public:
